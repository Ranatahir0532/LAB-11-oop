# 📁 C++ File Handling — OOP Lab 14

> Complete solution set for all exercises and tasks covering file I/O, OOP integration, binary files, and exception handling in C++.
...
```
lab14-file-handling/
├── topic1_exercise_name_age.cpp
├── topic1_task_append.cpp
├── topic2_exercise_even_numbers.cpp
├── topic2_task_count_lines.cpp
├── topic3_exercise_5_students.cpp
├── topic3_task_display_all_students.cpp
├── topic4_exercise_binary_write_multiple.cpp
├── topic4_task_binary_read_all.cpp
├── topic5_exercise_file_error_handling.cpp
├── topic5_task_file_plus_exception.cpp
└── final_task_student_management_system.cpp
```
...
| # | File | Concept |
|---|------|---------|
| 01 | `topic1_exercise_name_age.cpp` | `ofstream` · write name & age to file |
| 02 | `topic1_task_append.cpp` | `ios::app` · append without overwriting |
| 03 | `topic2_exercise_even_numbers.cpp` | `ifstream` · filter even numbers from file |
| 04 | `topic2_task_count_lines.cpp` | `getline` · count lines in a file |
| 05 | `topic3_exercise_5_students.cpp` | OOP + file · store 5 student records |
| 06 | `topic3_task_display_all_students.cpp` | OOP · `displayAllFromFile()` static method |
| 07 | `topic4_exercise_binary_write_multiple.cpp` | `ios::binary` · write objects to `.dat` |
| 08 | `topic4_task_binary_read_all.cpp` | Binary read · reconstruct & display all objects |
| 09 | `topic5_exercise_file_error_handling.cpp` | `try/catch` · handle file open failure |
| 10 | `topic5_task_file_plus_exception.cpp` | File I/O + full exception handling menu |
| 11 | `final_task_student_management_system.cpp` | Add · View · Search + persistent file storage |
...
# Example — Final Task
g++ -o final final_task_student_management_system.cpp
./final

# ⚠️  Topic 4: run the write program BEFORE the read program
g++ -o t4w topic4_exercise_binary_write_multiple.cpp && ./t4w
g++ -o t4r topic4_task_binary_read_all.cpp           && ./t4r
```
...
| Concept | Where Used |
|---------|-----------|
| `ofstream` / `ifstream` / `fstream` | Topics 1–3 |
| `ios::app` | Topic 1 Task |
| `ios::binary` + `write()` / `read()` | Topic 4 |
| OOP with file methods | Topics 3, Final |
| `try` / `catch` / `throw` | Topics 5, Final |
| Custom exception classes | Topic 5 Task |
| Menu-driven programs | Topics 3, 5, Final |
